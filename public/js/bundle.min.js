...
let currentSong=null,currentListeners=null,_utf8_encode=function(e){e=e.replace(/\r\n/g,"\n");for(var t="",r=0;r<e.length;r++){var n=e.charCodeAt(r);n<128?t+=String.fromCharCode(n):n>127&&n<2048?(t+=String.fromCharCode(n>>6|192),t+=String.fromCharCode(63&n|128)):(t+=String.fromCharCode(n>>12|224),t+=String.fromCharCode(n>>6&63|128),t+=String.fromCharCode(63&n|128))}return t},_utf8_decode=function(e){for(var t="",r=0,n=0,o=0,a=0;r<e.length;)(a=e.charCodeAt(r))<128?(t+=String.fromCharCode(a),r++):a>191&&a<224?(n=e.charCodeAt(r+1),t+=String.fromCharCode((31&a)<<6|63&n),r+=2):(n=e.charCodeAt(r+1),o=e.charCodeAt(r+2),t+=String.fromCharCode((15&a)<<12|(63&n)<<6|63&o),r+=3);return t};nowPlaying=()=>{const e=document.querySelector(".nowplaying");axios.get("https://icecast.freecatradio.com/status-json.xsl").then((t=>{let r=t.data.icestats.source,n="",o='<span class="mdi mdi-headphones"></span> ',a=0;r.title?(n=r.title,a=r.listeners):r[1]&&r[1].title?(o='<span class="mdi mdi-microphone-variant"></span> ',n=r[1].title,a=r[1].listeners):(n=r[0].title,a=r[0].listeners),a!==currentListeners&&(console.log("👂",a),currentListeners=a),currentSong!==n&&(e.classList.remove("fadeInUp","fadeOutUp"),e.classList.add("fadeOutUp"),currentSong=n,setTimeout((()=>{e.classList.remove("fadeInUp","fadeOutUp"),e.innerHTML=o+_utf8_decode(n),e.classList.add("fadeInUp")}),1e3))}))},setInterval((()=>nowPlaying()),1e4),nowPlaying();
var modes=["waves1","waves2","waves3","waves4","waves5","waves6"],colors=["e11d74","f86808","cf1b1b","394989","2b580c","21209c","34a853","5b5656","8105d8"],mode=modes[0],color=`#${colors[0]}`,context=null,src=null,canvas=document.getElementById("canvas"),d={width:canvas.width=window.innerWidth,height:canvas.height=window.innerHeight};function at(t,e){for(t.moveTo(e[0],e[1]),Ce=2;Ce<e.length-1;Ce+=2)t.lineTo(e[Ce],e[Ce+1])}function ct(t){var e=/^#?([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})$/i.exec(t);return e?{r:parseInt(e[1],16),g:parseInt(e[2],16),b:parseInt(e[3],16)}:null}function tt(t,e,h,o,i){t.beginPath(),t.arc(e,h,o,0,2*Math.PI),t.closePath(),t.fillStyle="rgba("+ct(color).r+", "+ct(color).g+", "+ct(color).b+", "+i+")",t.fill()}function it(t,e,h,o,i,r){if(t.beginPath(),at(t,function(t,e,h,o){e=void 0!==e?e:.5,h=h||!1,o=o||16;var i,r,l,n,c,a,d,g,s,u,f,m,b,p=[],v=[];for(p=t.slice(0),h?(p.unshift(t[t.length-1]),p.unshift(t[t.length-2]),p.unshift(t[t.length-1]),p.unshift(t[t.length-2]),p.push(t[0]),p.push(t[1])):(p.unshift(t[1]),p.unshift(t[0]),p.push(t[t.length-2]),p.push(t[t.length-1])),b=2;b<p.length-4;b+=2)for(m=0;m<=o;m++)l=(p[b+2]-p[b-2])*e,n=(p[b+4]-p[b])*e,c=(p[b+3]-p[b-1])*e,a=(p[b+5]-p[b+1])*e,f=m/o,d=2*Math.pow(f,3)-3*Math.pow(f,2)+1,g=-2*Math.pow(f,3)+3*Math.pow(f,2),s=Math.pow(f,3)-2*Math.pow(f,2)+f,u=Math.pow(f,3)-Math.pow(f,2),i=d*p[b]+g*p[b+2]+s*l+u*n,r=d*p[b+1]+g*p[b+3]+s*c+u*a,v.push(i),v.push(r);return v}(e,h,o,i)),r){t.beginPath();for(var l=0;l<e.length-1;l+=2)t.rect(e[l]-2,e[l+1]-2,4,4)}}function visualize(t){context||(context=new AudioContext,src=context.createMediaElementSource(t));var e=context.createAnalyser(),h=context.createGain(),o=document.getElementById("canvas").getContext("2d");const i=d.width;var r=0,l=[];for(Ce=0;Ce<512;Ce++){var n={};n.x=Math.floor(1920*Math.random()+1),n.y=Math.floor(1080*Math.random()+1),n.radius=Math.floor(1080*Math.random()/5+2),n.alpha=1,n.speed=Math.floor(50*Math.random()+30),l.push(n)}src.connect(h),h.connect(e),e.connect(context.destination),e.fftSize=1024;e.fftSize;var c=e.frequencyBinCount,a=new Uint8Array(c);!function t(){if(requestAnimationFrame(t),e.smoothingTimeConstant=.9,e.getByteFrequencyData(a),"waves1"==mode){o.clearRect(0,0,d.width,d.height),o.lineWidth=0,o.miterLimit=1;var h=[];o.beginPath(),o.moveTo(0,d.height);for(var n=0;n<i+20;n+=20){var c=Math.round(a.length/8*n/i);h.push(n),h.push(d.height-a[c]*d.height/255+2)}it(o,h,.5),o.lineTo(i,d.height),o.lineTo(0,d.height),o.fillStyle="rgba("+ct(color).r+", "+ct(color).g+", "+ct(color).b+", 0.1)",o.fill(),o.closePath(),o.beginPath(),o.moveTo(0,d.height),h=[];for(n=0;n<i+20;n+=20){c=Math.round(a.length/8*n/i);h.push(n),h.push(d.height-a[c+c]*d.height/255+2)}it(o,h,.5),o.lineTo(i,d.height),o.lineTo(0,d.height),o.fillStyle="rgba("+ct(color).r+", "+ct(color).g+", "+ct(color).b+", 0.15)",o.fill(),o.closePath(),o.beginPath(),o.moveTo(0,d.height),h=[];for(n=0;n<i+20;n+=20){c=Math.round(a.length/8*n/i);h.push(n),h.push(d.height-a[c+c+c]*d.height/255+2)}it(o,h,.5),o.lineTo(i,d.height),o.lineTo(0,d.height),o.fillStyle="rgba("+ct(color).r+", "+ct(color).g+", "+ct(color).b+", 0.2)",o.fill(),o.closePath(),o.beginPath(),o.moveTo(0,d.height),h=[];for(n=0;n<i+20;n+=20){c=Math.round(a.length/8*n/i);h.push(n),h.push(d.height-a[c+c+c+c]*d.height/255+2)}it(o,h,.5),o.lineTo(i,d.height),o.lineTo(0,d.height),o.fillStyle="rgba("+ct(color).r+", "+ct(color).g+", "+ct(color).b+", 0.25)",o.fill(),o.closePath()}else if("waves2"==mode){o.clearRect(0,0,d.width,d.height),o.lineWidth=2,o.miterLimit=1,o.beginPath(),o.moveTo(0,d.height);for(h=0;h<a.length/2;h+=1)o.lineTo(h*d.width/a.length*2,d.height-a[h]*d.height/255+2);o.strokeStyle="rgba("+ct(color).r+", "+ct(color).g+", "+ct(color).b+", 0.5)",o.stroke(),o.closePath()}else if("waves3"==mode){o.clearRect(0,0,d.width,d.height),o.lineWidth=1,o.miterLimit=1,o.beginPath();for(h=0;h<d.width;h+=3){n=Math.round(a.length/2*h/d.width);o.moveTo(h,d.height),o.lineTo(h,d.height-a[n]*d.height/255+2),o.strokeStyle="rgba("+ct(color).r+", "+ct(color).g+", "+ct(color).b+", 0.5)"}o.stroke()}else if("waves4"==mode){o.clearRect(0,0,d.width,d.height),o.lineWidth=1,o.miterLimit=1,o.beginPath(),o.moveTo(0,d.height);for(h=0;h<a.length/2;h+=1)o.lineTo(h*d.width/a.length*2,d.height-a[h]*d.height/255+2);o.lineTo(d.width,d.height),o.lineTo(0,d.height),o.fillStyle="rgba("+ct(color).r+", "+ct(color).g+", "+ct(color).b+", 0.5)",o.fill(),o.closePath()}else if("waves5"==mode){o.clearRect(0,0,d.width,d.height),o.lineWidth=3,o.lineCap="round",o.miterLimit=1,o.strokeStyle="rgba("+ct(color).r+", "+ct(color).g+", "+ct(color).b+", 0.2)";h=[];o.beginPath(),o.moveTo(0,d.height);for(n=0;n<i+20;n+=20){c=Math.round(a.length/8*n/i);h.push(n),h.push(d.height-a[c]*d.height/255+2)}it(o,h,.5),o.stroke(),o.closePath(),o.lineWidth=2,o.lineCap="round",o.miterLimit=1,o.strokeStyle="rgba("+ct(color).r+", "+ct(color).g+", "+ct(color).b+", 0.3)",o.beginPath(),o.moveTo(0,d.height),h=[];for(n=0;n<i+20;n+=20){c=Math.round(a.length/8*n/i);h.push(n),h.push(d.height-a[c+c]*d.height/255+2)}it(o,h,.5),o.stroke(),o.closePath(),o.lineWidth=2,o.lineCap="round",o.miterLimit=1,o.strokeStyle="rgba("+ct(color).r+", "+ct(color).g+", "+ct(color).b+", 0.4)",o.beginPath(),o.moveTo(0,d.height),h=[];for(n=0;n<i+20;n+=20){c=Math.round(a.length/8*n/i);h.push(n),h.push(d.height-a[c+c+c]*d.height/255+2)}it(o,h,.5),o.stroke(),o.closePath(),o.lineWidth=2,o.lineCap="round",o.miterLimit=1,o.strokeStyle="rgba("+ct(color).r+", "+ct(color).g+", "+ct(color).b+", 0.5)",o.beginPath(),o.moveTo(0,d.height),h=[];for(n=0;n<i+20;n+=20){c=Math.round(a.length/8*n/i);h.push(n),h.push(d.height-a[c+c+c+c]*d.height/255+2)}it(o,h,.5),o.stroke(),o.closePath()}else if("waves6"==mode){o.clearRect(0,0,d.width,d.height),r++;for(h=0;h<a.length/2;h++)tt(o,Math.cos(r/l[h].speed)*l[h].radius+l[h].x,Math.sin(r/l[h].speed)*l[h].radius+l[h].y,l[h].radius*a[h]/255,(a[h]/255/2+.5)/5)}}()}
...
const socket=io("https://api.freecatradio.com"),audio=document.getElementById("stream"),playBtn=document.getElementById("playBtn"),switchControls=document.getElementById("switchControls"),switchColor=document.getElementById("switchColor"),switchVisualiser=document.getElementById("switchVisualiser"),shuffleVisualiser=document.getElementById("shuffleVisualiser"),chatForm=document.getElementById("chat_form"),chatContent=document.getElementById("chat_content"),userName=document.getElementById("username"),messageInput=document.getElementById("message");let initChat=!1;document.querySelectorAll(".toggleChat").forEach((e=>{e.onclick=()=>{initChat||(socket.emit("request_history"),socket.on("history",(e=>{e.history.forEach((e=>{addLine(e)}));let t="Anonymous";localStorage.getItem("username")&&(t=localStorage.getItem("username"),socket.emit("change_username",t)),socket.emit("message",{username:"botcat",country:"botcat",message:`${t} joined`,created:new Date}),userName.innerHTML=t,initChat=!0,setInterval(updateTimestamps,6e4)}))),document.getElementById("chat").classList.toggle("active"),document.getElementById("chatControls").classList.toggle("active")}})),document.addEventListener("DOMContentLoaded",(()=>{document.getElementById("loading")&&(document.getElementById("loading").classList.add("animated","fadeOut"),setTimeout((()=>{document.getElementById("loading").remove()}),500))})),switchVisualiser.onclick=()=>{nextVisualizer()},shuffleVisualiser.onclick=()=>{randomVisualizer()},switchColor.onclick=()=>{nextColor()},playBtn.onclick=()=>{togglePlay()},userName.onclick=()=>{const e=localStorage.getItem("username")||"Anonymous";var t=prompt("Input your name",e);t.length>10&&(t=t.substring(0,10)),socket.emit("change_username",t),localStorage.setItem("username",t),userName.innerHTML=t},chatForm.onsubmit=()=>{const e=messageInput.value;return e.trim().length&&(messageInput.value="",e.length>255&&(e=e.substr(0,255)+"..."),socket.emit("message",{message:e})),!1},messageInput.onkeyup=()=>{messageInput.value&&socket.emit("typing")},socket.on("message",(e=>{addLine(e)})),togglePlay=()=>{document.getElementById("switchControls").classList.remove("fadeOut","fadeIn"),playBtn.classList.contains("is-playing")?(audio.pause(),audio.currentTime=0,playBtn.classList.remove("is-playing"),document.getElementById("canvas").classList.remove("active"),document.getElementById("overlay").classList.remove("active"),document.getElementById("switchControls").classList.remove("active")):(playBtn.classList.add("is-playing"),document.getElementById("canvas").classList.add("active"),document.getElementById("overlay").classList.add("active"),document.getElementById("switchControls").classList.add("active"),audio.play(),visualize(audio))},randomVisualizer=()=>{mode=modes[Math.floor(Math.random()*modes.length)],color=colors[Math.floor(Math.random()*colors.length)],color=`#${color}`},nextVisualizer=()=>{let e=modes.findIndex((e=>e===mode));e++,e>=modes.length&&(e=0),mode=modes[e]},nextColor=()=>{let e=colors.findIndex((e=>e===color.replace("#","")));e++,e>=colors.length&&(e=0),color=`#${colors[e]}`},scrollToBottom=()=>{setTimeout((()=>{const e=document.getElementById("chat_content");e&&(e.scrollTop=e.scrollHeight)}),50)},addLine=e=>{const t=moment(e.created).fromNow();chatContent.innerHTML+=`<p class="message"><span class="flag" style="background-image:url('/flags/${e.country}.svg')"></span><span class="username ${e.username}">${e.username}</span> ${e.message} <span class="time" value="${e.created}">${t}</span></p>`,scrollToBottom()},updateTimestamps=()=>{document.querySelectorAll(".time").forEach((e=>{e.textContent=moment(e.getAttribute("value")).fromNow()}))},moment.locale("es");
