...
const socket=io("https://api.freecatradio.com"),audio=document.getElementById("stream"),playBtn=document.getElementById("playBtn"),switchControls=document.getElementById("switchControls"),switchColor=document.getElementById("switchColor"),switchVisualiser=document.getElementById("switchVisualiser"),chatForm=document.getElementById("chat_form"),chatContent=document.getElementById("chat_content"),userName=document.getElementById("username"),messageInput=document.getElementById("message");let initChat=!1;document.querySelectorAll(".toggleChat").forEach((e=>{e.onclick=()=>{initChat||(socket.emit("request_history"),socket.on("history",(e=>{e.history.forEach((e=>{addLine(e)}));let t="Anonymous";localStorage.getItem("username")&&(t=localStorage.getItem("username"),socket.emit("change_username",t)),socket.emit("message",{username:"botcat",country:"botcat",message:`${t} joined`,created:new Date}),userName.innerHTML=t,initChat=!0,setInterval(updateTimestamps,6e4)}))),document.getElementById("chat").classList.toggle("active"),document.getElementById("chatControls").classList.toggle("active")}})),document.addEventListener("DOMContentLoaded",(()=>{moment.locale("es"),document.getElementById("loading")&&(document.getElementById("loading").classList.add("animated","fadeOut"),setTimeout((()=>{document.getElementById("loading").remove()}),500))})),switchVisualiser.onclick=()=>{nextVisualizer()},switchColor.onclick=()=>{nextColor()},playBtn.onclick=()=>{togglePlay()},userName.onclick=()=>{const e=localStorage.getItem("username")||"Anonymous";var t=prompt("Input your name",e);t.length>10&&(t=t.substring(0,10)),socket.emit("change_username",t),localStorage.setItem("username",t),userName.innerHTML=t},chatForm.onsubmit=()=>{const e=messageInput.value;return e.trim().length&&(messageInput.value="",e.length>255&&(e=e.substr(0,255)+"..."),socket.emit("message",{message:e})),!1},messageInput.onkeyup=()=>{messageInput.value&&socket.emit("typing")},socket.on("message",(e=>{addLine(e)})),togglePlay=()=>{document.getElementById("switchControls").classList.remove("fadeOut","fadeIn"),playBtn.classList.contains("is-playing")?(audio.pause(),audio.currentTime=0,playBtn.classList.remove("is-playing"),document.getElementById("canvas").classList.remove("active"),document.getElementById("overlay").classList.remove("active"),document.getElementById("switchControls").classList.remove("active")):(playBtn.classList.add("is-playing"),document.getElementById("canvas").classList.add("active"),document.getElementById("overlay").classList.add("active"),document.getElementById("switchControls").classList.add("active"),audio.play(),visualize(audio))},nextVisualizer=()=>{let e=modes.findIndex((e=>e===mode));e++,e>=modes.length&&(e=0),mode=modes[e]},nextColor=()=>{let e=colors.findIndex((e=>e===color.replace("#","")));e++,e>=colors.length&&(e=0),color=`#${colors[e]}`},scrollToBottom=()=>{setTimeout((()=>{const e=document.getElementById("chat_content");e&&(e.scrollTop=e.scrollHeight)}),50)},addLine=e=>{const t=moment(e.created).fromNow();chatContent.innerHTML+=`<p class="message"><span class="flag" style="background-image:url('/flags/${e.country}.svg')"></span><span class="username ${e.username}">${e.username}</span> ${e.message} <span class="time" value="${e.created}">${t}</span></p>`,scrollToBottom()},updateTimestamps=()=>{document.querySelectorAll(".time").forEach((e=>{e.textContent=moment(e.getAttribute("value")).fromNow()}))};
